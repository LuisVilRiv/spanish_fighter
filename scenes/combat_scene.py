...
class CombatView(BaseView):
    ...
    def _setup_ui(self):
        ...
        try:
            ...
            self.sprite_jugador = arcade.Sprite(tex, center_x=self.jugador_x + sprite_size//2,
                                                center_y=y_combat + sprite_size//2)
        ...
        try:
            ...
            self.sprite_enemigo = arcade.Sprite(tex, center_x=self.enemigo_x + sprite_size//2,
                                                center_y=y_combat + sprite_size//2)
        ...
        msg_y = btn_y + 90
        msg_height = 100
        self.msg_rect = (50, msg_y, w-100, msg_height)  # (x, y, width, height)
    ...
    def on_draw(self):
        ...
        if self.sprite_jugador:
            self.sprite_jugador.draw()
        if self.sprite_enemigo:
            self.sprite_enemigo.draw()
        ...
        arcade.draw_rectangle_filled(x + w//2, y + h//2, w, h, (20, 20, 30, 200))

        # Dibujar elementos de UI (etiquetas y botones principales)
        for elem in self.ui_elements:
            if hasattr(elem, 'draw'):
                elem.draw()

        # Dibujar botones de habilidades
        for btn in self.habilidad_buttons:
            btn.draw()
