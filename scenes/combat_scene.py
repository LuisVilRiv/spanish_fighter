...
class CombatView(BaseView):
    def __init__(self, app, jugador, enemigo):
        ...
        # ── Historial propio: acumulamos cada ResultadoTurno aquí ─────────
    ...
    def _setup_ui(self):
        ...
        #
        #  [FOOTER_PAD]
        #  [ACTION_H  ]  ← botones principales / postcombate
        #  [GAP       ]
        #  [HAB_H     ]  ← botones de habilidad + cancelar  (banda propia)
        #  [GAP       ]
        #  [MSG_H     ]  ← log de mensajes
        #  [GAP       ]
        #  [arena     ]  ← sprites + barras
        #  [HEADER_H  ]  ← indicador de turno
        #
        ...
        # ── Botones de habilidades (banda propia, SIN solapamiento) ───────
    ...
    def on_update(self, delta_time):
        super().on_update(delta_time)  # desactiva _block_next_press tras el primer frame
        if self.ia_timer > 0:
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if self._block_next_press:
            return
        # Enrutamiento exclusivo: habilidades abiertas → solo esos botones.
        if self.mostrando_habilidades:
            for btn in self.habilidad_buttons:
